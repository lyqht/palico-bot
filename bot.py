...
from settings import DEMO_STATUS
...
def formatted_text_for_trello_task(task, members):
    if task.due:
        formatted_text = f"    🔗[link]({task.shortUrl}) _Due {DATE_HELPER.convert_datetime_to_string(task.due)}_.\n"
    else:
        formatted_text = f"    🔗[link]({task.shortUrl})\n"
    if len(members):
...
def gantt_curr_week_handler(update, context: CallbackContext):
    ...
    message = [
        "Meow~ these are the tasks for the next 2 weeks on Gantt Chart 📚"
    ]
...
def trello_tasks_handler(update, context: CallbackContext):
    ...
    buttons = [
        InlineKeyboardButton(members[member_id], callback_data=member_id)
...
def trello_member_selected_callback(update, context: CallbackContext):
    ...
    message = [
        TRELLO_QUERY_COMPLETE_MESSAGE + f"*{member}* 🤟	 \n" + separator_line +
        "\n"
    ]
    ...
    for i in range(len(tasks)):
        ...
        partners_on_same_task = [
            members[id] for id in task.members if id != requested_member_id
        ]
...
def weekly_reminder(update, context: CallbackContext):
    curr_week = DATE_HELPER.current_week_number()
    WEEKLY_REMINDER_INTRO_MESSAGE = (
        f"It is almost *Week {curr_week+1}*, and here are some updates!\n")
    ...
    message.append("")
    message.append(f"*Trello Tasks for Week {curr_week}-{curr_week+1}*")
    for i in range(len(tasks)):
        task = tasks[i]
        message.append(f"{i+1}. {task.name}\n" +
                       formatted_text_for_trello_task(
                           task, trello.translate_ids_to_names(task.members)))
